...
n_years_later <- function(member_data, years) {
  ...
  for (x in member_data){
    x <- list(list(name = x$name, age = as.numeric(x$age) + years))
...
```{r}
# Write your code here
setwd("/Users/stevetai/Documents/GitHub/hw4")
cute_cute <- as.tibble(read.csv("week3Rclass.csv"))
# Do not modify the code below
cute_cute
# Should print out
#> # A tibble: 85 x 6
...
gend <- c()
for (y in Q_Q$gender){
  if (gsub("^\\s+|\\s+$", "", as.character(y)) == "M"){
    gend <- append(gend, as.integer(1))
  }
  else if (gsub("^\\s+|\\s+$", "", as.character(y)) == "F"){
    gend <- append(gend, as.integer(2))
  }
  else{
    gend <- append(gend, as.integer(0))
  }
}
Q_Q$GPA <- gpa
Q_Q$gender <- gend
...
# Write your code here
fav <- c()
for (z in cute_cute$q_self){
  if (z < 70){
    fav <- append(fav, "一代一代")
  }
  else{
    fav <- append(fav, "根本可愛動物區")
  }
}
cute_cute["favorite_word"] <- fav
# Do not modify the code below
cute_cute
# Should print out
#> # A tibble: 85 x 7
